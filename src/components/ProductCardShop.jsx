...
export default function ProductCard(data) {
    ...
    return(        
        <div className="flex flex-col items-center font-montserrat gap-4 overflow-hidden">            
            <div className="w-[239px] h-[300px] max-lg:w-[318px] max-lg:h-[427px] bg-center bg-cover" style={{ backgroundImage: `url(${data.products.images[0].url})` }}></div>
            <div className="flex flex-col items-center gap-3">
                <h5 className="font-bold text-base text-[#252B42]"><Link to="/product-detail">{data.products.name}</Link></h5>
                <a href="" className="font-bold text-sm text-[#737373]"><Link to="/product-detail">English Department</Link></a>
                <div className="flex flex-row gap-3">
                    <span className="font-bold text-base text-[#BDBDBD]">${data.products.price}</span>
